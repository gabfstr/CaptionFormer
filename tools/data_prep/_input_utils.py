...
#   from https://github.com/google-research/scenic/blob/main/scenic/projects/densevoc/input_utils.py
...
from tools.data_prep import _transforms as transforms
  ...
  if image_set == 'train':
    return transforms.Compose(
        [transforms.RandomHorizontalFlip(),
         ...
         transforms.FixedSizeCropWithAdditionalKeys(
...
#         decode_sharded_names(dataset_path))
...
densecap_sequence_feature_description = {
    ...
    'objects/caption': tf.io.VarLenFeature(tf.string),
    'objects/category': tf.io.VarLenFeature(tf.string)
}

densecap_sequence_feature_description_w_msk = {
    'image/encoded': tf.io.FixedLenSequenceFeature([], tf.string),
    'objects/bbox': tf.io.VarLenFeature(tf.float32),
    'objects/track_id': tf.io.VarLenFeature(tf.int64),
    'objects/segment_id': tf.io.VarLenFeature(tf.int64),
    'objects/caption': tf.io.VarLenFeature(tf.string),
    'objects/category': tf.io.VarLenFeature(tf.string),
    'objects/segmentation': tf.io.VarLenFeature(tf.string),  # Add segmentations
}
...
densecap_context_feature_description = {
    ...
    'image_ids': tf.io.VarLenFeature(dtype=tf.int64),
    'data_path': tf.io.VarLenFeature(dtype=tf.string),
    'data_path': tf.io.VarLenFeature(dtype=tf.string),
    'fps': tf.io.VarLenFeature(dtype=tf.float32),
    'frame_count': tf.io.VarLenFeature(dtype=tf.int64),
    'frame_ids': tf.io.VarLenFeature(dtype=tf.int64),
}

densecap_context_feature_description_smit = {
    'video_id': tf.io.VarLenFeature(dtype=tf.string),
    'video/captions': tf.io.VarLenFeature(dtype=tf.string),
    'image_ids': tf.io.VarLenFeature(dtype=tf.int64),
    'data_path': tf.io.VarLenFeature(dtype=tf.string),
    'data_path': tf.io.VarLenFeature(dtype=tf.string),
...
def decode_and_sample_video_example(
    ...
    use_category = True, use_masks=False):
  """Convert custom tfrecord into tfds builder format."""
  print("context_feature keys:", context_feature.keys())
  print("seq_feature keys:", seq_feature.keys())
#   print("context:", context_feature)
#   data_path = tf.sparse.to_dense(context_feature['data_path'])
#   fps = tf.sparse.to_dense(context_feature['fps'])
#   print("data_path:", data_path)
#   print("fps:", fps)
#   print("video captions :", tf.sparse.to_dense(context_feature['video/captions']))
#   raise ValueError("Debugginggg")
  ...
  if use_category:
    category = tf.map_fn(
        tf.sparse.to_dense,
        seq_feature['objects/category'], back_prop=False, dtype=tf.string)
  else:
    # category = tf.zeros((tf.shape(bbox)[0], tf.shape(bbox)[1]), dtype=tf.float32)
    # not zeros but a string with 'object' size of (bbox)[0], (bbox)[1]
    category = tf.fill((tf.shape(bbox)[0], tf.shape(bbox)[1]), 'object')
  if use_masks:
    # print("seq features keys:", seq_feature.keys())
    masks = tf.map_fn(
        tf.sparse.to_dense,
        seq_feature['objects/segmentation'], back_prop=False, dtype=tf.string)
  ...
  if num_frames > 0:
    ...
    category = tf.gather(category, inds)
    if use_masks:
  ...
  bbox = tf.map_fn(
      lambda x: decode_boxes(x, tf.shape(images)[1:3]),
  ...
  out_dict = {
      ...
      'data_path': tf.sparse.to_dense(context_feature['data_path']),
      'fps': tf.sparse.to_dense(context_feature['fps']),
      'frame_count': tf.sparse.to_dense(context_feature['frame_count']),
      'frame_ids': tf.sparse.to_dense(context_feature['frame_ids']),
...
#             x, coco_feature_description))
#     ds = ds.map(coco_decode_example)
#   return ds
