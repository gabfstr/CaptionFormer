...
def process_video_record(
    ...
    data_path, fps, frame_count, frame_ids):
  ...
  for anns in all_objects:
    ...
    # print("boxes", [x['bbox'] for x in anns])
    # print("captions", [x['caption'] for x in anns])
    # print("object categories", [x['category'] for x in anns])
    categories.append(_bytes_feature(
      [str_to_bytes(x['category']) for x in anns]))
  ...
  feature = {
      ...
      'data_path': _bytes_feature([str_to_bytes(data_path)]),
      'fps': _float_feature([fps]),
      'frame_count': _int64_feature([frame_count]),
      'frame_ids': _int64_feature(frame_ids),
  }
  seq_feature = {
      'image/encoded': tf.train.FeatureList(feature=all_images),
      'objects/bbox': tf.train.FeatureList(feature=boxes),
      'objects/category': tf.train.FeatureList(feature=categories),
      # 'objects/category_id': tf.train.FeatureList(feature=cat_ids
...
class ConvertSeqExamples(object):
  ...
  def process(self, example):
    ...
    if vid not in self.vid2stg_anns:
      return []
    # print("vid keys", example.context.feature.keys())
    ...
    assert len(feature_list['image/encoded'].feature) == frame_count, (
        '{} {}'.format(len(feature_list['image/encoded'].feature), frame_count))
    ...
    for frame_id in frame_ids:
      ...
      for x in vidor_anns_frame:
        ...
        objs[x['tid']] = {
            'caption': '', 'category':'', 'question': '', 'bbox': bbox,
      ...
      for (traj_id, traj) in vidstg_anns:
        if frame_id >= traj['temporal_gt']['begin_fid'] and frame_id < traj[
          ...
          for category in traj['subject/objects']:
            cat_tid = category['tid']
            # print("cat_tid", cat_tid)
            # print("objs", objs)
            # assert cat_tid in objs
            if cat_tid in objs:
              objs[cat_tid]['category'] = category['category']
          for q in traj['questions']:
            q_tid = q['target_id']
    ...
    out_example = process_video_record(
        all_images, width, height, video_id, all_objects,
        video_captions, video_questions, all_ids, data_path, 
        fps, frame_count, frame_ids)
    return [out_example]
