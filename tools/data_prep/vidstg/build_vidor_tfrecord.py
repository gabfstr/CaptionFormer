...
def create_dataset(ann_path, video_dir, output_path):
  ...
  to_skip_per_meet = 0
  to_skip = to_skip_per_meet
  print("temporary : skipping {} shards when meeting existing files".format(to_skip))
  ...
  count = 0
  for _, feats in anns.items():
    count += 1
    
    if skipping==True and ((count % num_examples_per_shard != 0) or (shard_id >= num_shards - 1)):
        continue
    feat=feats[0]
    # print("feat : ", feat)
    # print("len(feat) : ", len(feat))
    # print("feat[0]keys : ", feat[0].keys())
    if not skipping:
    ...
    if (count % num_examples_per_shard == 0) and shard_id < num_shards - 1:
        ...
        if gfile.exists(output_path):
            print('Already exists', output_path)
            # Skip till next shard
            skipping = True
            to_skip = to_skip_per_meet
            continue
        elif to_skip>0:
            print("skipping {} more shards".format(to_skip))
            to_skip-=1
            skipping = True
            continue
        else:
