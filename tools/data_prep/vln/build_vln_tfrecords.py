...
def process_video_record(
  ...
  data_path = video_info['ytid']+'/'
  frame_ids = [int(os.path.basename(x).split('.')[0]) for x in file_names]

  images = []
  boxes, segmentations, track_ids, captions = [], [], [], []
  for i in range(NUM_IMAGES):
    # file_name = image_dir + '/' + file_names[i]
    file_name = os.path.join(image_dir, file_names[i])
    img_string = gfile.GFile(file_name, 'rb').read()
    images.append(_bytes_feature([img_string]))
    boxes_frame, masks_frames, ids_frame, caption_frame = [], [], [], []
    ...
    # print("bbox:", bbox)
    # print("ids_frame:", ids_frame)
    # print("caption_frame:", caption_frame)
    # raise ValueError("bbox: {}, ids_frame: {}, caption_frame: {}".format(
        # bbox, ids_frame, caption_frame))
    mask_bytes = [json.dumps(mask).encode('utf-8') if mask is not None else b'' for mask in masks_frames]
    ...
    segmentations.append(_bytes_feature(mask_bytes))  # Add RLE masks
  ...
  seq_feature = {
      ...
      'objects/segmentation': tf.train.FeatureList(feature=segmentations),  # Add segmentations
