...
#   from https://github.com/fanghaook/OVFormer/blob/main/evaluate/bursteval.py
...
class BURSTeval:
    ...
    def _prepare(self):
        ...
        def _toMask(anns, coco):
            ...
            for ann in anns:
                ...
                # l = [int(a) for a in ann['areas'] if a and 'areas' in ann]
                # if len(l)==0:
                #   ann['avg_area'] = 0
                # else:
        ...
        def iou_seq(d_seq, g_seq):
            ...
            for d, g in zip(d_seq, g_seq):
                if d and g:
                    ...
                    u += maskUtils.area(d)
            if not u > .0:
                self.logger.info("Mask sizes in video {} and category {} may not match!".format(vidId, catId))
            iou = i / u if u > .0 else .0
            return iou
        ious = np.zeros([len(d), len(g)])
        for i, j in np.ndindex(ious.shape):
            ious[i, j] = iou_seq(d[i], g[j])
        #self.logger.info(vidId, catId, ious.shape, ious)
        return ious
    ...
    def summarize(self):
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            if ap == 1:
                cs = s[:,:,np.array(self.cocoGt.common_categories)-1]
                us = s[:,:,np.array(self.cocoGt.uncommon_categories)-1]
            else:
                cs = s[:,np.array(self.cocoGt.common_categories)-1]
                us = s[:,np.array(self.cocoGt.uncommon_categories)-1]
            ...
            if len(cs[cs>-1])==0:
                ...
                mean_cs = np.mean(cs[cs>-1])

            if len(us[us>-1])==0:
                mean_us = -1
            else:
                mean_us = np.mean(us[us>-1])

            self.logger.info(iStr.format(titleStr, typeStr, iouStr, areaRng, maxDets, mean_s))
            self.logger.info(iStr.format(titleStr, typeStr + 'b', iouStr, areaRng, maxDets, mean_bs))
            self.logger.info(iStr.format(titleStr, typeStr + 'n', iouStr, areaRng, maxDets, mean_ns))
            self.logger.info(iStr.format(titleStr, typeStr + 'c', iouStr, areaRng, maxDets, mean_cs))
            self.logger.info(iStr.format(titleStr, typeStr + 'u', iouStr, areaRng, maxDets, mean_us))
            return mean_s
        def _summarizeDets():
            stats = np.zeros((12,))
