...
#   from https://github.com/fanghaook/OVFormer/blob/main/train_net.py
"""
...
from captionformer import (
    COCODenseDVOCNewBaselineDatasetMapper,
        ...
        # coco instance segmentation lsj new baseline
            ...
            if cfg.MODEL.MASK_FORMER.MASK_CAPTIONING==True:
                if cfg.MODEL.MASK_FORMER.BOX_MODE_ON == True :
                    mapper = COCODenseDVOCNewBaselineDatasetMapper(cfg, True, no_crop=True)
                elif cfg.MODEL.MASK_FORMER.USE_BOXES == True :
                    mapper = COCODenseDVOCNewBaselineDatasetMapper(cfg, True, use_boxes=True)
                else :
                    mapper = COCODenseDVOCNewBaselineDatasetMapper(cfg, True)
            else :
                # print("\n\n\n\n\n experimental")
                # mapper = COCODenseDVOCNewBaselineDatasetMapper(cfg, True, no_crop=True)
                if cfg.MODEL.MASK_FORMER.BOX_MODE_ON == True and "vidstg" in cfg.DATASETS.TRAIN[0]:
                    mapper = COCOInstanceNewBaselineDatasetMapper(cfg, True, no_crop=True)
                elif cfg.MODEL.MASK_FORMER.BOX_MODE_ON == True :
                    mapper = COCOInstanceNewBaselineDatasetMapper(cfg, True, box_only=True)
                elif cfg.MODEL.MASK_FORMER.USE_BOXES == True :
                    mapper = COCOInstanceNewBaselineDatasetMapper(cfg, True, use_boxes=True)
                else :
...
def setup(args):
    ...
    if "lvis_v1" in dataset:
        cfg.MODEL.CAPTIONING_HEAD.FEATURE_MAPPING = "lvis"
    elif "visual_genome" in dataset:
        cfg.MODEL.CAPTIONING_HEAD.FEATURE_MAPPING = "vg"
