...
def custom_register_lvis_instances(name, metadata, json_file, image_root):
    """
    """
    DatasetCatalog.register(name, lambda: custom_load_vidstg_img_json(
...
def _get_vidstg_img_instances_meta(key):
    assert len(VIDSTG_CATEGORIES) == 80
    cat_ids = [k["id"] for k in VIDSTG_CATEGORIES]
    assert min(cat_ids) == 1 and max(cat_ids) == len(
        cat_ids
    ), "Category ids are not in [1, #categories], as expected"
    # Ensure that the category list is sorted by id
    lvis_categories = sorted(VIDSTG_CATEGORIES, key=lambda x: x["id"])
...
def custom_load_vidstg_img_json(json_file, image_root, dataset_name=None):
    '''
    Modifications:
      use `file_name`
    ...
    if len(lvis_api.dataset['categories']) == 80:
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            obj_cat_name = lvis_api.cats[anno['category_id']]['name']
            if 'caption' in anno :
                if anno['caption'] is not None:
                    obj["caption"] = anno['caption']
                else :
                    obj["caption"] = "An instance of {} is visible.".format(obj_cat_name.split("_(")[0].replace("_", " "))
            else:
                obj["caption"] = "An instance of {} is visible.".format(obj_cat_name.split("_(")[0].replace("_", " "))
...
_CUSTOM_SPLITS_LVIS = {
    "vidstg_train_2_frames_img": ("VidSTG/video/", "VidSTG/annotations/train_instances_2_frames_img.json"),
    "vidstg_val_2_frames_img": ("VidSTG/video/", "VidSTG/annotations/val_instances_2_frames_img.json"),
    "vidstg_train_200_frames_img": ("VidSTG/video/", "VidSTG/annotations/train_instances_200_frames_img.json"),
    "vidstg_val_200_frames_img": ("VidSTG/video/", "VidSTG/annotations/val_instances_200_frames_img.json"),
}
...
for key, (image_root, json_file) in _CUSTOM_SPLITS_LVIS.items():
    custom_register_lvis_instances(
        key,
        _get_vidstg_img_instances_meta(key),
