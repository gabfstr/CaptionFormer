...
def custom_load_lvis_json(json_file, image_root, dataset_name=None):
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        record["dataset"] = img_dict.get("dataset", '')
...
_CUSTOM_SPLITS_LVIS = {
    "coco_msk_train": ("coco/", "lvis/coco_mask_train.json"),
    "coco+vg_box_train": ("coco/", "lvis/coco+vg_box_train.json"),
    "lviscap_coco_train": ("coco/", "lvis/lviscap_v1_train+coco.json"),
    "visual_genome_cap_train": ("coco/", "lvis/vg_box_train.json"),
    "visualgenome": ("VisualGenome/", "VisualGenome/annotations/visualgenome_coco_format.json"),
    ...
    "lviscap_train": ("coco/", "lvis/lviscap_v1_train.json"),
    ...
    "lviscap_val": ("coco/", "lvis/lviscap_v1_val.json"),
}

_root = os.getenv("DETECTRON2_DATASETS", "datasets")
for key, (image_root, json_file) in _CUSTOM_SPLITS_LVIS.items():
    if key == "visualgenome" or key == "visual_genome_cap_train" or key == "visual_genome_cap_test" or key == "coco+vg_box_train" or key == "coco_msk_train":
        custom_register_lvis_instances(
            key,
            get_lvis_instances_meta('lvis_v1_train'),
