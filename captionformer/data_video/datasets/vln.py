...
def load_vln_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
...
def register_vln_instances(name, metadata, json_file, image_root):
    """
    """
    assert isinstance(name, str), name
    assert isinstance(json_file, (str, os.PathLike)), json_file
    assert isinstance(image_root, (str, os.PathLike)), image_root
    DatasetCatalog.register(name, lambda: load_vln_json(
        json_file, image_root, name, extra_annotation_keys=['instance_id'], 
    ))
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="vln", **metadata
    )
