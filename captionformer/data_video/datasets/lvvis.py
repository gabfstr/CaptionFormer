...
"""
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        if not "test" in dataset_name:
            if len(anno_dict_list) == 0:
                # logger.warning(
                #     "Video {} has no annotations. "
                #     "Please check the annotation file.".format(vid_dict["id"])
                # )
                continue
        ...
        feat_batch_size=64
        batch_id = [i//feat_batch_size for i in range(record["length"])]
        record["feature_paths"] = [
            os.path.join(
                image_root.replace("JPEGImages", "features"),
                str(video_id).zfill(5),
                f"feat_{video_id:05d}_{i+1}of{1+record['length']//feat_batch_size}.pth"
            ) for i in batch_id]
        ...
        for frame_idx in range(record["length"]):
            ...
            for anno in anno_dict_list:
                ...
                if "caption" in anno:
                    ...
                    # obj["caption"] = None
                    obj_cat_name = ytvis_api.cats[obj['category_id']]['name']
                    obj["caption"] = "An instance of {} is visible.".format(obj_cat_name.split("_(")[0].replace("_", " "))
                # obj_cat_name = ytvis_api.cats[obj['category_id']]['name']
                # obj["caption"] = "An instance of {}.".format(obj_cat_name)
        ...
        # if len(video_objs) > 0:
            # logger.warning(
            #     "Video {} has no valid annotations. "
            #     "Please check the annotation file.".format(video_id)
            # )
