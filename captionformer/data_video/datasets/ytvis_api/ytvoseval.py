...
class YTVOSeval:
    ...
    def _prepare(self):
        ...
        def _toBoxes(anns):
            for ann in anns:
                for i, a in enumerate(ann['bbox']):
                    if a:
                        # Convert from w,y,w,h to x1,y1,x2,y2
                        x, y, w, h = a
                        ann['bbox'][i] = [x, y, x + w, y + h]
                l = [a for a in ann['areas'] if a]
                if len(l)==0:
                    ann['avg_area'] = 0
                else:
    ...
    def computeIoU(self, vidId, catId):
        ...
        if p.iouType == 'bbox':
            def iou_seq(d_seq, g_seq, is_crowd):
                ious=[]
                for d, g in zip(d_seq, g_seq):
                    if d and g:
                        # print("d :",d)
                        # print("g : ",g)
                        iou = maskUtils.iou([d], [g], [is_crowd])
                        # print("iou : ", iou)
                        iou = iou[0][0]
                        ious.append(iou)
                    else :
            ...
            # print("len d, len g :", len(d), len(g))
            ious = np.zeros([len(d), len(g)])
            for i, j in np.ndindex(ious.shape):
                ious[i,j]=iou_seq(d[i],g[j], iscrowd[j])
                # print("ious i j :", ious[i,j])
                # raise Exception("stop")
            # raise Exception('bbox iou computation not implemented')
