...
class YTVOS:
    ...
    def loadRes(self, resFile):
        ...
        if 'segmentations' in anns[0] or 'bbox' in anns[0]:
                ...
                if "segmentations" in ann:
                    for seg in ann['segmentations']:
                        ...
                        if seg:
                            ...
                            if len(ann['bbox']) < len(ann['areas']):
                                ann['bbox'].append(maskUtils.toBbox(seg))
                        else:
                            ann['areas'].append(None)
                            if len(ann['bbox']) < len(ann['areas']):
                                ann['bbox'].append(None)
                else:
                    for idx, b in enumerate(ann['bbox']):
                        if b :
                            ann['areas'].append(b[2]*b[3])
