...
#   from https://github.com/fanghaook/OVFormer/blob/main/ovformer/data_video/lvvis_eval_video.py
...
class LVVISEvaluator_video(DatasetEvaluator):
    """
    ...
    def __init__(
        ...
        *,
        ...
        assert self.eval_mode in ["bbox", "segm"], "Only support 'bbox' or 'segm' evaluation mode"
        ...
        if hasattr(self._metadata, "thing_dataset_id_to_contiguous_id"):
            ...
            for result in predictions:
                ...
                if category_id >= num_classes:
                    error = f"A prediction has class={category_id}, but the dataset only has {num_classes} classes and predicted class id should be in [0, {num_classes - 1}]."
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    if "pred_captions" in outputs:
        pred_captions = outputs["pred_captions"]
    else :
        pred_captions = [None] * len(scores)
    
    if "pred_scores_per_frame" in outputs:
        scores_per_frame = outputs["pred_scores_per_frame"]
    else :
        scores_per_frame = [None] * len(scores)
    
    
    if "pred_boxes" in outputs:
        # print("\n\n\n Using box mode for evaluation \n\n\n")
        boxes = outputs["pred_boxes"]
        print("temporary adding per frame scores")
        ytvis_results = []
        for instance_id, (s, l, b, c, spf) in enumerate(zip(scores, labels, boxes, pred_captions, scores_per_frame)):
        ...
        ytvis_results = []
        for instance_id, (s, l, m, c, spf) in enumerate(zip(scores, labels, masks, pred_captions, scores_per_frame)):
            segms = [
...
def _evaluate_predictions_on_coco(
    ...
    assert eval_mode in ["bbox", "segm"], "Only support 'bbox' or 'segm' evaluation mode"
    ...
    coco_eval = YTVOSeval(coco_gt, coco_dt, iouType=eval_mode)
