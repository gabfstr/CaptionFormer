...
#   from https://github.com/fanghaook/OVFormer/blob/main/ovformer/data_video/burst_eval.py
...
from .datasets.ytvis_api.burst import BURST
    ...
    def __init__(
        ...
        *,
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            self._ytvis_api = BURST(json_file)
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    for instance_id, (s, l, m) in enumerate(zip(scores, labels, masks)):
        m = sem_seg_postprocess(m[None,...], image_size, height, width, mid_image_size)[0].detach().cpu()
        m = (m > 0).float().numpy()
        segms = [
