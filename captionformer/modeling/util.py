...
#   from https://github.com/fanghaook/OVFormer/blob/main/ovformer/modeling/util.py
...
def load_class_freq(
    ...
    if path=='datasets/metadata/lvis_v1_train_cat_info.json':
        cat_info = torch.tensor(
            [(c['image_count'] if c['frequency'] != 'r' else 0) for c in sorted(cat_info, key=lambda x: x['id'])])
    else:
        cat_info = torch.tensor(
            [(c['image_count'] if c['frequency'] != 'n' else 0) for c in sorted(cat_info, key=lambda x: x['id'])])
