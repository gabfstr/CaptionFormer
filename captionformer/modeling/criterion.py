...
#   from https://github.com/fanghaook/OVFormer/blob/main/ovformer/modeling/criterion.py
"""
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 num_points, oversample_ratio, importance_sample_ratio, empty_object_coeff=0.4):
        ...
        # Box mode : box loss instead of mask loss
        if 'boxes' in self.losses and not 'masks' in self.losses:
            self.box_mode = True
        else :
            self.box_mode = False 
        ...
        empty_weight[-1] = 0
        self.empty_object_coeff = empty_object_coeff
        empty_object_weight = torch.ones(2)
        empty_object_weight[-1] = empty_object_coeff
    ...
    def loss_labels(self, outputs, targets, indices, num_masks):
        ...
        ignore_cls_mask = torch.zeros((B, Q), dtype=torch.bool, device=src_logits.device)
        for b_idx, (src, tgt) in enumerate(indices):
            dataset = targets[b_idx].get("dataset", "")
            if dataset == "visualgenome":
                ignore_cls_mask[b_idx, src] = True

        keep_cls_mask = ~ignore_cls_mask  # Invert mask
        ...
        if self.freq_weight is not None:
            loss_ce = 1.7 * torch.sum(
                loss_ce * weight * (1 - target_object_classes[:, :, None]) * keep_cls_mask[:, :, None]
            ) / (keep_cls_mask.sum() * self.num_classes + 1e-6)
        else:
            loss_ce = 1.7 * torch.sum(
                loss_ce * (1 - target_object_classes[:, :, None]) * keep_cls_mask[:, :, None]
            ) / (keep_cls_mask.sum() * self.num_classes + 1e-6)
        # if self.freq_weight is not None :
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        def _assert_finite(t, name):
            if not torch.isfinite(t).all():
                raise RuntimeError(f"{name} contains NaN/Inf (value range {t.min()} – {t.max()})")
        ...
        # filter out VG instances
        if any(t.get("dataset", "") == "visualgenome" for t in targets):
            keep = torch.ones(point_logits.size(0), dtype=torch.bool, device=point_logits.device)
            offset = 0
            for b_idx, (src_ids, _) in enumerate(indices):
                n = len(src_ids)
                if targets[b_idx].get("dataset", "") == "visualgenome":
                    keep[offset:offset + n] = False
                offset += n

            point_logits = point_logits[keep]
            point_labels = point_labels[keep]
            num_masks    = float(keep.sum().item())

        if num_masks == 0:
            zero = src_masks.sum() * 0.0      # requires_grad True
            return {"loss_mask": zero, "loss_dice": zero}
        

        loss_mask = sigmoid_ce_loss_jit(point_logits, point_labels, num_masks)
        _assert_finite(loss_mask, "loss_mask")

        loss_dice = dice_loss_jit(point_logits, point_labels, num_masks)
        _assert_finite(loss_dice, "loss_dice")

        losses = {"loss_mask": loss_mask, "loss_dice": loss_dice}
        # losses = {
        #     "loss_mask": sigmoid_ce_loss_jit(point_logits, point_labels, num_masks),
        #     "loss_dice": dice_loss_jit(point_logits, point_labels, num_masks),
        # }
