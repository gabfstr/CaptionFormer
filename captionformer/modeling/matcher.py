...
#   from https://github.com/fanghaook/OVFormer/blob/main/ovformer/modeling/matcher.py
"""
...
class HungarianMatcher(nn.Module):
    ...
    def box_memory_efficient_forward(self, outputs, targets):
        """Memory-friendly matching for boxes instead of masks."""
        bs, num_queries = outputs["pred_logits"].shape[:2]
        indices = []
        ...
        for b in range(bs):
            ...
            cost_class = -out_prob[:, tgt_ids]  # Negative probabilities for target classes

            # Extract predicted and target boxes
            out_boxes = outputs["pred_boxes"][b]  # [num_queries, 4]
            tgt_boxes = targets[b]["boxes"].to(out_boxes)  # [num_targets, 4]
            # print("out_boxes : ", out_boxes.shape)
            ...
            cost_giou = 1 - generalized_box_iou(
                box_clamp_xyxy(box_xywh_to_xyxy(out_boxes)),
                box_xywh_to_xyxy(tgt_boxes)
            )
            ...
            C = C.reshape(num_queries, -1).cpu() # n_src_boxes, n_tgt_boxes

            # Hungarian matching with the cost matrix
            indices.append(self.linear_sum_assignment_with_inf(C))
