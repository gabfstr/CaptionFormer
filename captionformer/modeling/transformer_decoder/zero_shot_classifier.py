...
#   from https://github.com/fanghaook/OVFormer/blob/main/ovformer/modeling/transformer_decoder/zero_shot_classifier.py
...
class ZeroShotClassifier(nn.Module):
    ...
    def forward(self, x, features_clip, classifier=None):
        ...
        features_clip = torch.stack(features_clip, dim=1)
        ...
        if classifier is not None:
            ...
            zs_weight = self.zs_weight  # (512, k)
        if self.norm_weight:
            x = self.norm_temperature * F.normalize(x, p=2, dim=2)
        bs, qn, _ = x.shape
        x = x.reshape(bs * qn, -1)  # (b*100, 512)
        x = torch.mm(x, zs_weight)  # (b*100, k)
        x = x.reshape(bs, qn, -1)  # (b,100,k)
        if self.use_bias:
            x = x + self.cls_bias
        return x
