...
#   from https://github.com/fanghaook/OVFormer/blob/main/ovformer/ovformer_model_video.py
...
class CaptionFormerVideo(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            for i in range(len(batched_inputs)):
                file_name = 'datasets/' + batched_inputs[i]['file_names'][0].split('datasets/', 1)[1]
                feature_clip = self.clip_train[file_name].to(self.device)
            ...
            for i in range(batched_inputs[0]['length']):
                file_name = 'datasets/' + batched_inputs[0]['file_names'][i].split('datasets/', 1)[1]
                feature_clip = self.clip_val[file_name].to(self.device).to(torch.float32)
    ...
    def instance_inference(self, mask_cls, mask_object_cls, mask_pred, image_size, mask_per_frame_cls, mask_per_frame_object_cls):
        ...
        mask_per_frame_object_scores = mask_per_frame_object_scores[torch.arange(Q),:,labels_per_image.detach().cpu()]
