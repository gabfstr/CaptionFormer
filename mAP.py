...
#   from https://github.com/fanghaook/OVFormer/blob/main/mAP.py
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dt', default='output/inference/results.json')
    parser.add_argument('--et', default='("lvvis_val",)') # ("burst_val",)
    parser.add_argument('-subclasses', '--subclasses', action='store_true')

    args = parser.parse_args()
    dt_path=args.dt
    # output folder is before "inference"
    root_folder = args.dt.split("/inference/")[0]
    output_file = os.path.join(root_folder, "results.txt")
    ...
    if 'lvvis' in eval_type:
        ...
        if args.subclasses:
            gt_path='./datasets/LVVIS/val/val_instances_200_classes.json'
